...
def lambda_handler(event, context):
    ...
    for coin in coinArray:
        ...
        if not coin.__eq__("["):
            ...
            # if ticker.__contains__("BTC") and not ticker.__contains__("ETHBTC"):
            #     coinNameSplit = ticker.split("BTC")
            #     for split in coinNameSplit:
            #         if not split.__eq__(""):
            #             coinName = split
            #     price = str(float(price) * BTCUSD)
            #     getTickerPriceChanges(ticker, price, coinName)
            ...
            if not ticker.__contains__("ETH") and not ticker.__contains__("BTC"):
                price = coin.split('u\'price\': u\'')[1].split("\'},")[0]
                # getTickerPriceChanges(ticker, price, ticker)
                # print("\nNo BTC/ETH conversion for " + ticker + " " + price + "\n")
    
    buyTopBreakout()

def query_table(filter_key=None, filter_value=None):
    if filter_key and filter_value:
        dateTime = str(datetime.now() - timedelta(hours = .8))
...
#get the holding you own most of right now
def getCurrentHold():
    ...
    for balance in balances:
        balanceAmount = float(balance['free'])
        priceInEth = 0
        if balanceAmount > 1 and balance['asset'] != "ETH":
            try:
                params = {"symbol": balance['asset'] + "ETH"}
                # print("searching for price of " + balance['asset'] + "ETH")
                r = str(requests.get('https://api.binance.com/api/v1/ticker/price', params=params).json())
                time.sleep(.5)
                # print(r)
                price = float(r.split("price\': u\'")[1].split("\'")[0]) * balanceAmount
                # print(balance['asset'] + " " + str(price))
                if price > topBalance:
                    topBalance = price
                    holding = balance['asset']
                    print("top hold of " + holding + " is " + topBalance + " balance amt " + balanceAmount)
            except Exception as e:
                i = 0
                #print("Error " + balance['asset'])

    global holdETH
    holdETH = topBalance
    return holding

def sellForGainOrHardLoss(newBuyPrice):
    oldBuyPrice = query_table("symbol", "MYHOLD")['Items'][0]['price-USD']
    print("Hold value was " + str(oldBuyPrice) + " and is now " + str(holdETH))
    print(float(holdETH)/float(oldBuyPrice))
    # print(holdETH > oldBuyPrice)
    
    # if the price has gone up 2.5% or more sell or if down 25% sell
    if float(holdETH)/float(oldBuyPrice) >= 1.025 or float(holdETH)/float(oldBuyPrice) <= .75:
        table.put_item(
            Item={
            'symbol': "MYHOLD",
            'price-USD': str(newBuyPrice),
            'timeStamp': "blah"
        })
        print("SELL HOLD")
        return True
    ...
    for balance in balances:
        ...
        if balanceAmount > 0:
            ...
            while bought == False and count < 4:
                ...
                response = binanceClient.order_market_sell(symbol=balance['asset'] + "ETH", quantity=balanceAmount)
                # print(response)
                if not str(response).__contains__("failure: LOT_SIZE"):
                    bought = True
                else:
                    balanceAmount = round(balanceAmount, rounding)
                    rounding = rounding - 1
...
def sellHoldForEth():
    ...
    if hold is not "ETH":
        binanceClient = Client(api_key, api_secret)
        balances = balances = binanceClient.get_account()['balances']
        for balance in balances:
            balanceAmount = int(float(balance['free']))
            if balanceAmount > 0 and balance['asset'] == hold:
                print("selling " + str(balanceAmount) + " " +  balance['asset'])
                response = binanceClient.order_market_sell(symbol=balance['asset'] + "ETH", quantity=balanceAmount)
...
def getTickerPriceChanges(priceUSD, priceETH, coinName):
    ...
    Item={
    'symbol': coinName,
    'price-USD': str(priceUSD),
    'timeStamp': time 
    })

    #price from at most 1/2 hrs ago
    try:
        priceLastQuery = query_table("symbol", coinName)['Items'][0]['price-USD']
    except Exception as e:
        # print("error with ticker " + coinName)
        return
    percentChange = str((float(priceUSD)/float(priceLastQuery) - 1) * 100)

    # print(coinName + " " + percentChange)
    price6HoursAgo = query_table_by_hours(6, "symbol", coinName)['Items'][0]['price-USD']
    price24HoursAgo = query_table_by_hours(24, "symbol", coinName)['Items'][0]['price-USD']

    if float(percentChange) >= 0.01 and float(percentChange) <= 5 and priceLastQuery > price6HoursAgo and priceLastQuery > price24HoursAgo:
        print("The price of " + coinName + " was " + str(price6HoursAgo) + " 6 hrs ago and " + str(price24HoursAgo) + " 24 hrs ago. " + str(percentChange) + "% change past 5 min.")
        global breakouts
...
#if the last hold has gone up 5% or more or down 25% of more sell it for the next highest
#% change coin between now and last iteration if it's no longer the current hold. 
def buyTopBreakout():
    # print(breakouts + "\n")
    best = ""
    bestPrice = 0.0
    ...
    if currentHold != best and bestPrice != 0:
        ...
        if holdIsETH != True:
            sell = sellForGainOrHardLoss(quantity * bestPrice)
        else:
            sell = True
            table.put_item(
            Item={
            'symbol': "MYHOLD",
            'price-USD': str(holdETH),
            'timeStamp': "blah"
            })
        if sell == True:
            sellHoldForEth()
